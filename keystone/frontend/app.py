...
with st.sidebar:
    ...
    st.markdown("### 📝 LLM Generated Text")
    ...
    # Verify button
    verify_button = st.button("🚀 Verify Facts", type="primary", use_container_width=True)
...
# Main header
st.markdown('<h1 class="main-header">🔑 Keystone</h1>', unsafe_allow_html=True)
st.markdown("### Automated Fact-Checking & Attribution for LLMs")
st.markdown("---")

# Logic
if not uploaded_file:
    st.info("👈 Upload a source document in the sidebar to begin")
    ...
    st.info("👈 Paste LLM-generated text in the sidebar to verify")

elif verify_button:
    # Check API key if needed by ClaimExtractor (Rule-based fallback exists, but warning is better)
    if not st.session_state.get('api_key'):
        st.warning("⚠️ No API Key provided. Claim extraction will rely on rule-based fallback (less accurate).")
    ...
    with st.spinner("📄 Processing source document..."):
        try:
            ...
            st.success(f"✅ Processed document: {len(chunks)} chunks created.")
    ...
    with st.spinner("🗄️ Building knowledge base..."):
        try:
            ...
            st.success(f"✅ Indexed {stats.get('total_chunks', 0)} chunks into '{collection_name}'")
        except Exception as e:
    ...
    if not results:
        ...
        col4.metric("⚠️ Unverifiable", unverifiable)
        col5.metric("🎯 Trust Score", f"{trust_score:.0f}%")
        ...
        # Split Screen View
        st.markdown("---")
        st.subheader("📑 Visual Document View")
        ...
        with right_col:
            st.markdown("**📄 Source Document Context**")
